    ...
    def function_block_declaration(self, ):
        ...
        try:
            try:
                ...
                # action start
                self.system = get_system(name.text, ((function_block_type1 is not None) and [
                    self.input.toString(function_block_type1.start, function_block_type1.stop)] or [None])[0])
                # action end


                # action start
                self.type_ = ((function_block_type1 is not None) and [
                    self.input.toString(function_block_type1.start, function_block_type1.stop)] or [None])[0]
                # action end
    ...
    def sconclusion(self, ):
        ...
        try:
            try:
                ...
                # action start
                s = i.text if i is not None else ((n is not None) and [self.input.toString(n.start, n.stop)] or [None])[
                    0]
                # action end
        ...
        try:
            try:
                ...
                if alt34 == 1:
                    ...
                    # action start
                    fset = get_fset('Singleton',
                                    [float(((n is not None) and [self.input.toString(n.start, n.stop)] or [None])[0])])
                    # action end
                    ...
                    # action start
                    fset = get_fset('Singleton', self.scope[
                        ((v is not None) and [self.input.toString(v.start, v.stop)] or [None])[0]])
                    # action end
            ...
            try:
                ...
                while True:  # loop36
                    ...
                    if alt36 == 1:
                        ...
                        # action start
                        points.append((float(((x is not None) and [self.input.toString(x.start, x.stop)] or [None])[0]),
                                       float(
                                           ((y is not None) and [self.input.toString(y.start, y.stop)] or [None])[0])))
                        # action end
        ...
        try:
            try:
                ...
                if alt39 == 1:
                    ...
                    # action start
                    param = (float(((x is not None) and [self.input.toString(x.start, x.stop)] or [None])[0]),
                             float(((y is not None) and [self.input.toString(y.start, y.stop)] or [None])[0]))
                    # action end
    ...
    def range(self, ):
        ...
        try:
            try:
                ...
                # action start
                interval = Interval(float(((x is not None) and [self.input.toString(x.start, x.stop)] or [None])[0]),
                                    float(((y is not None) and [self.input.toString(y.start, y.stop)] or [None])[0]))
                # action end
